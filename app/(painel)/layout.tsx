...
import { buscarConfiguracao, contarNavegacao, usuarioAtivo } from '@/lib/consultas';
...
const LayoutPainel = async ({ children }: { children: ReactNode }) => {
  ...
  const [contagens, configuracao] = await Promise.all([
    contarNavegacao(usuario.nome),
    buscarConfiguracao(),
  ]);

  return (
    <Casca usuario={usuario} contagens={contagens} configuracao={configuracao}>
      {children}
    </Casca>
  );
};
